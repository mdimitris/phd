...
# ==========================================
# FINAL MODEL TRAINING (FOR SHAP)
# ==========================================

print("\nTraining final XGBoost model for SHAP...")

from sklearn.model_selection import GroupShuffleSplit
import joblib

# Split once at patient level (no leakage)
gss = GroupShuffleSplit(n_splits=1, test_size=0.2, random_state=42)

for train_idx, val_idx in gss.split(clean_df, groups=clean_df["subject_id"]):
    train_df = clean_df.iloc[train_idx].copy()
    val_df   = clean_df.iloc[val_idx].copy()

# Train final XGBoost model
final_trainer = lstm.SepsisTrainer(
    features=features,
    label_col=l_col_val,
    model_type="xgb",
    early_stopping_rounds=50,
...
metrics = final_trainer.train_tree_from_splits(train_df, val_df)

print("Final model metrics:", metrics)

# ==========================================
# SAVE MODEL + VALIDATION DATA
# ==========================================
...
print(type(final_trainer.model))
joblib.dump(final_trainer.model, "models/xgb_24h_final.pkl")
val_df.to_parquet("data/val_df_24h.parquet")
